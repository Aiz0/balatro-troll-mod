...
return {
    add_level = function(card)
        ...
        local level_stats = level_stat_sheet[card.ability.extra.level]
    ...
    calculate = function(self, card, context)
        ...
        if context.discard
            ...
            local damage = context.other_card.base.nominal
            local health = context.other_card.base.id - 1
            card.ability.extra.health = card.ability.extra.health - damage
            if card.ability.extra.health <= 0 then
                card:shatter();
            end

            if card.ability.extra.attack < health then
    ...
    loc_vars = function(self, info_queue, card)
        ...
        if card.ability.extra.discards > 1 then key = self.key.."_multiple" end
        return { 
            key = key,
            vars = {
